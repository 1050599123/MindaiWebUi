		  ...
		  @Parameters({"BaseUrl"})//读取testng.xml参数
		   ...
		   @DataProvider(name="longinData")
			public Object[][] loginData()
			{
			   ...
			   return ExcelReadUtil.case_data_excel(2, 1, 4, 1, 6,filePath);
			}
		   
	    @Test(description="注册失败用例",dataProvider = "longinData")
	    public void Login( String regName,String regPass,String imgCode,String smsCode,String inviter,String message) throws Exception, DocumentException{ 
			//代替testng参数化的方法
			String BaseUrl= XmlReadUtil.getTestngParametersValue("testng.xml","BaseUrl");			
			//调用登录方法
			RegistrationAction loginAction=new RegistrationAction(BaseUrl+"/register.html",regName,regPass,imgCode,smsCode,inviter);
			action.sleep(1);   
			//设置检查点
