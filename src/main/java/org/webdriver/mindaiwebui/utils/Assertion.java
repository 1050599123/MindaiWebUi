...
public class Assertion extends TestBaseCase {
	 ...
	 */
	public static  void VerityTextdb  (String Phone ,String Message)
	{  
		UserTable userTable = new UserTable();
		String number="【Assert验证】:"+"数据库中是否出现"+"【"+"预期值："+Phone+"】"+"字符串";
		log.info(Message+":"+number);
		try {
			Assert.assertTrue(userTable.CheckSelectUserForMoblePhoneResult(Phone));
			AssertPassLog();
			assertInfolList.add(Message+number+":pass");
			messageList.add(Message+":pass");
		} catch (Error f) {
			// TODO: handle exception
			AssertFailedLog();
			errors.add(f);
			errorIndex++;
			assertInfolList.add(Message+number+":failed");
			messageList.add(Message+":failed");
			Assertion.snapshotInfo();
		}
